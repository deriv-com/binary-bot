          ...
          gapi.client.drive.files
            ...
            .then(response => {
              try {
                ...
                const error = new TrackJSError('GoogleDrive', translate('Unrecognized file format'), err);
      ...
      Promise.all(!this.is_authorized ? this.authorise : [])
        .then(() => {
          ...
          gapi.client.drive.files.list({ q: 'trashed=false' }).then(response => {
            const folder = response.result.files.find(file => file.mimeType === 'application/vnd.google-apps.folder');
      ...
      const savePickerCallback = data => {
        if (data.action === google.picker.Action.PICKED) {
          ...
          xhr.onload = () => {
            ...
            const error = new TrackJSError('GoogleDrive', translate('There was an error processing your request'), xhr);
            globalObserver.emit('Error', error);
            reject(error);
          };
          xhr.send(form_data);
