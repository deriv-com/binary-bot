              ...
              return xmlFields.some(
                f =>
                  ...
                  availableCategories.some(
                    category =>
                      category[1] === tradeTypeCategory && Object.assign(f, { textContent: tradeTypeCategory })
                  )
              );
            } catch (e) {
              return false;
...
export const getUnattachedMandatoryPairs = () => {
  ...
  const unattachedPairs = config.mandatoryBlockPairs.filter(pair =>
    presentMandatoryMainBlocks.find(block => block.type === pair.parentBlock && !hasChildOfType(block, pair.childBlock))
  );
        ...
        addLoadersFirst(xml, header).then(
          () => {
            Array.from(xml.children)
              .filter(
                block => block.getAttribute('type') === 'tick_analysis' || isProcedure(block.getAttribute('type'))
              )
      ...
      if (!isNew) {
        ...
        $.ajax({
          ...
          .fail(e => {
            if (e.status) {
              reject(
                Error(`${translate('An error occurred while trying to load the url')}: ${e.status} ${e.statusText}`)
              );
            } else {
              reject(
                Error(translate('Make sure \'Access-Control-Allow-Origin\' exists in the response from the server'))
              );
            }
