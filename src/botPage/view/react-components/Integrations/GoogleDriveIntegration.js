...
const GoogleDriveIntegration = () => {
  ...
  return (
    <div className="integration input-row last">
      ...
      <div className="right">
        <a onClick={() => google_drive_util.authorise()} className={!is_gd_logged_in ? 'button' : 'button-disabled'}>
          <span id="connect-google-drive">{translate('Connect')}</span>
        </a>
        <a onClick={() => google_drive_util.logout()} className={is_gd_logged_in ? 'button' : 'button-disabled'}>
